...
from . import auth
from .forms import RegisterEmailForm, OTPForm, SetPasswordForm, LoginForm, UpdateProfileForm, ChangePasswordForm, SellerOnboardForm
from .services import generate_and_send_otp, verify_otp, create_user_after_password, update_user_password
...
def set_password():
    ...
    if form.validate_on_submit():
        success, result = create_user_after_password(email, phone, name, form.password.data)
        if success:
            ...
            flash(result, 'danger')
    return render_template('auth/set_password.html', form=form, email=email)
...
def profile():
    ...
    onboard_form = SellerOnboardForm()
    ...
    if onboard_form.submit.data and onboard_form.validate():
        current_user.upi_id = onboard_form.upi_id.data
        db.session.commit()
        flash('UPI ID saved successfully! You can now receive funds for your books.', 'success')
        return redirect(url_for('auth.profile'))

    return render_template('auth/profile.html', form=form, password_form=password_form, onboard_form=onboard_form)
